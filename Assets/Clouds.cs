...
public class Clouds : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            //cloudAppear(cloudObjects[a]);
            a++;
        }
        else if (hash(hash(Time.deltaTime + 1) + seed) > 284)
        {
            //cloudDisappear(cloudObjects[b]);
            b++;
        }
    ...
    void cloudAppear(GameObject Cloud)
    {
    ...
    void cloudDisappear(GameObject Cloud)
    {

        Color color = Cloud.GetComponent<Renderer> ().material.color;
        for (float i = 255; i >= 0; i -= 5)
        {
            color -= new Color(255,255,255,255-i);
            Cloud.GetComponent<Renderer> ().material.color = color;
            wait();
        }
        //color.a -= Time.deltaTime * 25;
    }
